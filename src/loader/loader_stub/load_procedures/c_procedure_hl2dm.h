...
class c_procedure_hl2dm : public c_procedure_base{
public:
  ...
  bool write_signatures() override{
   ...
   set_module(HASH("client.dll"));
   {
     ...
     add_signature(HASH("vgui_drawhud_retaddr"), SIG("\x8A\x45\x00\x8B\xCB"), false);
     ...
     add_signature(HASH("set_abs_origin"), SIG("\x55\x8B\xEC\x56\x57\x8B\x7D\x00\x8B\xF1\xE8"), false);
     ...
     add_signature(HASH("cweaponpistol_updatepenaltytime_addr"), SIG("\x55\x8B\xEC\x83\xEC\x00\x56\x57\x8B\xF9\xE8\x00\x00\x00\x00\x8B\xF0\x85\xF6\x0F\x84"), false);
   }
   ...
   set_module(HASH("engine.dll"));
   {
     ...
     add_signature(HASH("cl_runprediction"), SIG("\x55\x8B\xEC\x83\xEC\x00\x8B\x0D\x00\x00\x00\x00\x33\xD2\x53\x33\xDB\x89\x55\x00\x89\x5D\x00\x8B\x01\x89\x55\x00\x56\x85\xC0\x74\x00\x68\x00\x00\x00\x00\x68\x00\x00\x00\x00\x68\x00\x00\x00\x00\x6A"), false);
  ...
  bool write_hooks() override{
    ...
    set_module(HASH("client.dll"));
    {
      ...
      add_hook(HASH("create_move_hook"), SIG("\x55\x8B\xEC\x8B\x55\x00\x85\xD2\x74\x00\x81\xC1"), false);
      ...
      //////v24 = sub_101C61D0();
      ////// (*(*v24 + 156))(v24, a2); <-----
      add_hook(HASH("do_post_screen_space_effects_hook"), SIG("\xB0\x00\xC2\x00\x00\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\x55\x8B\xEC\x56\xFF\x75"), false);
    ...
    set_module(HASH("engine.dll"));
    {
      ...
      //\x55\x8B\xEC\x83\xEC\x00\x53\x8B\x5D\x00\x56\x8B\xF1\x89\x75\x00\x85\xDB\x0F\x84
      add_hook(HASH("fire_event_hook"), SIG("\x55\x8B\xEC\x81\xEC\x00\x00\x00\x00\x8B\x15\x00\x00\x00\x00\x53\x57"), false);
      //// Be in-game first
      ...
      add_hook(HASH("cclient_state_process_fix_angle_hook"), SIG("\x55\x8B\xEC\xF3\x0F\x10\x1D\x00\x00\x00\x00\x83\xEC"), false);
    }

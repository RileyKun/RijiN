...
class c_procedure_syn : public c_procedure_base{
public:
  ...
  bool handle_globaldata(){
    ...
    set_module(HASH("client.dll"));
    {
      ...
      u32 global_data_ptr = (u32)find_signature(HASH("client.dll"), HASH(".text"), SIG("\xA1\x00\x00\x00\x00\x8B\x40\x00\x89\x86\x00\x00\x00\x00\xC6\x86\x00\x00\x00\x00\x00\xF3\x0F\x2C\x8E"), false, 0x1);
  ...
  bool write_signatures() override{
   ...
   set_module(HASH("client.dll"));
   {
     ...
     add_signature(HASH("firebullet_raytrace_retaddr"), SIG("\xA1\x00\x00\x00\x00\x83\x78\x00\x00\x74\x00\x51\xC7\x04\x24\x00\x00\x00\x00\x6A"), false);
    // add_signature(HASH("cweaponpistol_updatepenaltytime_addr"), SIG("\x55\x8B\xEC\x51\x56\x57\x8B\xF1\xE8\x00\x00\x00\x00\x8B\xF8\x85\xFF\x0F\x84\x00\x00\x00\x00\x8B\x17\x8B\xCF\x8B\x92\x00\x00\x00\x00\xFF\xD2\x84\xC0\x74"), false);
   }
   ...
   set_module(HASH("engine.dll"));
   {
     ...
     add_signature(HASH("steam3client_addr"), SIG("\xE8\x00\x00\x00\x00\x83\x78\x00\x00\x74\x00\xE8\x00\x00\x00\x00\x83\x78\x00\x00\x74\x00\xE8\x00\x00\x00\x00\x83\x38\x00\x74\x00\xE8\x00\x00\x00\x00\x8D\x55\x00\x52\x8B\x08\x8B\x01\xFF\x50\x00\x8D\x45\x00\x50\x56\xE8\x00\x00\x00\x00\x83\xC4\x00\x8B\x45"), true);
   }

   set_module(HASH("server.dll"));
   {    
     //add_signature(HASH("server_firebullet_bad_retaddr1"), SIG("\x5B\x8B\xE5\x5D\xC2\x00\x00\xCC\xCC\xCC\xCC\xCC\x55\x8B\xEC\x83\xEC\x00\x56\x8B\xF1\x8B\x86\x00\x00\x00\x00\xC1\xE8\x00\xA8\x00\x74\x00\xE8\x00\x00\x00\x00\x8B\x45\x00\xF3\x0F\x10\x8E"), false);
   }
  ...
  bool write_hooks() override{
    ...
    set_module(HASH("client.dll"));
    {
      ...
      add_hook(HASH("override_view_hook"), SIG("\x55\x8B\xEC\x83\xEC\x00\x56\xE8\x00\x00\x00\x00\x8B\xF0\x85\xF6\x0F\x84"), false);
      ...
      add_hook(HASH("base_animating_setupbones_hook"), SIG("\x55\x8B\xEC\x81\xEC\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x53\x8B\xD9\x8B\x0D\x00\x00\x00\x00\x56\x8B\x75\x00\x89\xB5"), false);
      ...
      //v24 = sub_101C61D0();
      // (*(*v24 + 156))(v24, a2); <-----
      //add_hook(HASH("do_post_screen_space_effects_hook"), SIG("\xB0\x00\xC2\x00\x00\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\xCC\x55\x8B\xEC\x56"), false);

      add_hook(HASH("set_dormant_hook"), SIG("\x55\x8B\xEC\x53\x8B\x5D\x00\x56\x8B\xF1\x8B\x06"), false);

      add_hook(HASH("fire_bullets_hook"), SIG("\x53\x8B\xDC\x83\xEC\x00\x83\xE4\x00\x83\xC4\x00\x55\x8B\x6B\x00\x89\x6C\x24\x00\x8B\xEC\x81\xEC\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x56\x8B\xF1\x57\x8B\x7B\x00\x89\xB5"), false);
    }
    ...
    set_module(HASH("engine.dll"));
    {
      ...
      add_hook(HASH("fire_event_hook"), SIG("\x55\x8B\xEC\x6A\x00\x6A\x00\xFF\x75\x00\xE8\x00\x00\x00\x00\x5D"), false);
      //// Be in-game first
      ...
      add_hook(HASH("cclient_state_process_fix_angle_hook"), SIG("\x55\x8B\xEC\x8B\x45\x00\x83\xEC\x00\xF3\x0F\x10\x15"), false);

      add_hook(HASH("engine_trace_ray_hook"), SIG("\x53\x8B\xDC\x83\xEC\x00\x83\xE4\x00\x83\xC4\x00\x55\x8B\x6B\x00\x89\x6C\x24\x00\x8B\xEC\xB8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x56"), false);
    }
    ...
    set_module(HASH("server.dll"));
    {
     // add_hook(HASH("server_base_entity_fire_bullet_hook"), SIG("\x53\x8B\xDC\x83\xEC\x00\x83\xE4\x00\x83\xC4\x00\x55\x8B\x6B\x00\x89\x6C\x24\x00\x8B\xEC\x81\xEC\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x56\x8B\xF1\x57\x8B\x7B\x00\x89\xB5"), false);
    }

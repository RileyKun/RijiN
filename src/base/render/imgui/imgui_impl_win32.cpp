...
static BOOL _IsWindowsVersionOrGreater(WORD major, WORD minor, WORD)
{
	...
	if (RtlVerifyVersionInfoFn == NULL)
		if (HMODULE ntdllModule = ::GetModuleHandleA(XOR("ntdll.dll")))
			RtlVerifyVersionInfoFn = (PFN_RtlVerifyVersionInfo)GetProcAddress(ntdllModule, XOR("RtlVerifyVersionInfo"));
    if (RtlVerifyVersionInfoFn == NULL)
        return FALSE;
...
void ImGui_ImplWin32_EnableDpiAwareness()
{
    ...
    {
        static HINSTANCE user32_dll = ::LoadLibraryA(XOR("user32.dll")); // Reference counted per-process
        if (PFN_SetThreadDpiAwarenessContext SetThreadDpiAwarenessContextFn = (PFN_SetThreadDpiAwarenessContext)::GetProcAddress(user32_dll, XOR("SetThreadDpiAwarenessContext")))
        {
    ...
    {
        static HINSTANCE shcore_dll = ::LoadLibraryA(XOR("shcore.dll")); // Reference counted per-process
        if (PFN_SetProcessDpiAwareness SetProcessDpiAwarenessFn = (PFN_SetProcessDpiAwareness)::GetProcAddress(shcore_dll, XOR("SetProcessDpiAwareness")))
        {
...
float ImGui_ImplWin32_GetDpiScaleForMonitor(void* monitor)
{
    ...
    if (_IsWindows8Point1OrGreater())
    {
		...
		if (GetDpiForMonitorFn == NULL && shcore_dll != NULL)
            GetDpiForMonitorFn = (PFN_GetDpiForMonitor)::GetProcAddress(shcore_dll, XOR("GetDpiForMonitor"));

...
// System includes
#include <ctype.h>      // str_utils::toupper
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    const ImGuiID id = window->GetID(XOR("#image"));
    PopID();
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (!overlay)
    {
        ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf), XOR("%.0f%%"), fraction * 100 + 0.01f);
        overlay = overlay_buf;
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held);
    const ImGuiID popup_id = ImHashStr(XOR("##ComboPopup"), 0, id);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), XOR("##Combo_%02d"), g.BeginPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    bool value_changed = DragScalar(XOR("##min"), ImGuiDataType_Float, v_current_min, v_speed, &min_min, &min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    value_changed |= DragScalar(XOR("##max"), ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
    ...
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? XOR("%08X") : XOR("%d");
...
/*
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ColorButton(XOR("##current"), col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text(XOR("Original"));
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            if (ColorButton(XOR("##original"), ref_col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2)))
            {
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        ImGuiID close_button_id = GetIDWithSeed(XOR("#CLOSE"), NULL, id);
    ...
    if (values_count >= values_count_min)
    {
        ...
        if (hovered && inner_bb.Contains(g.IO.MousePos))
        {
            ...
            if (plot_type == ImGuiPlotType_Lines)
                SetTooltip(XOR("%d: %8.4g\n%d: %8.4g"), v_idx, v0, v_idx + 1, v1);
            else if (plot_type == ImGuiPlotType_Histogram)
                SetTooltip(XOR("%d: %8.4g"), v_idx, v0);
            idx_hovered = v_idx;
        }
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open = BeginViewportSideBar(XOR("##MainMenuBar"), viewport, ImGuiDir_Up, height, window_flags);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    if (ArrowButtonEx(XOR("##<"), ImGuiDir_Left, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
        select_dir = -1;
    window->DC.CursorPos = ImVec2(x + arrow_button_size.x, tab_bar->BarRect.Min.y);
    if (ArrowButtonEx(XOR("##>"), ImGuiDir_Right, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
        select_dir = +1;
    PopStyleColor(2);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo(XOR("##v"), NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    const ImGuiID close_button_id = p_open ? GetIDWithSeed(XOR("#CLOSE"), NULL, id) : 0;
    bool just_closed;
    bool text_clipped;
    ...
    if (text_clipped && g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > g.TooltipSlowDelay && IsItemHovered())
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) && !(tab->Flags & ImGuiTabItemFlags_NoTooltip))
            SetTooltip(XOR("%.*s"), (int)(FindRenderedTextEnd(label) - label), label);

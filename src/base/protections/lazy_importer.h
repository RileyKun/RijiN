...
#pragma once
#if defined(DEV_MODE)
#if !defined(VAC_BYPASS_MODULE) && !defined(DXGIDMP)
    #if defined(LOADER_CLIENT) || defined(LOADER_STUB) || defined(INTERNAL_CHEAT)
        #define ANTI_REVERSE_FUNC_OBFUSCATION
    #endif
#endif
#endif

#if defined(ANTI_REVERSE_FUNC_OBFUSCATION) && !defined(INTERNAL_CHEAT) && !defined(DXGIDMP)
...
#define I_FN_DEF(name) ::li::detail::lazy_function<::li::detail::khash(#name), name>()

#define I_MODULE(name) ::li::detail::lazy_module<::li::detail::khash(name)>()
...
#else
#define I(name) name
#define I_FN_DEF(name) name
#define I_MODULE(name) name
#endif

...
void c_antiaim::run(){
  ...
  if(!antiaim_enabled){
    if(config->hvh.antiaim_headshotdanger){
      if(!headshot_class && antiaim_active_headshot_threat <= math::time()){
        if(antiaim_toggle || !config->hvh.antiaim_usekey) // Make it in color if it would turn on if a threat was detected.
          global->info_panel->add_entry(INFO_PANEL_ANTIAIM, config->hvh.antiaim_legit ? WXOR(L"LEGIT (NO THREAT)") : WXOR(L"RAGE (NO THREAT)"), config->hvh.antiaim_legit ? INFO_PANEL_LEGIT_CLR : INFO_PANEL_RAGE_CLR);
        else // Otherwise make it white.
          global->info_panel->add_entry(INFO_PANEL_ANTIAIM, config->hvh.antiaim_legit ? WXOR(L"LEGIT (NO THREAT)") : WXOR(L"RAGE (NO THREAT)"));
      }
    }
    else
      global->info_panel->add_entry(INFO_PANEL_ANTIAIM, WXOR(L"INACTIVE"));

    return;
  }

  global->info_panel->add_entry(INFO_PANEL_ANTIAIM, config->hvh.antiaim_legit ? WXOR(L"LEGIT") : WXOR(L"RAGE"), config->hvh.antiaim_legit ? INFO_PANEL_LEGIT_CLR : INFO_PANEL_RAGE_CLR);

  if(!fake_latency->is_synced())
    global->info_panel->add_entry(INFO_PANEL_ANTIAIM, WXOR(L"FAKE LATENCY SYNC"), INFO_PANEL_WARNING_CLR);
...
void c_antiaim::pitch(c_base_player* localplayer){
  ...
  // Fix config errors.
  {
    ...
    if(!config->hvh.antiaim_pitch_dynamic_real_up && !config->hvh.antiaim_pitch_dynamic_real_down)
      config->hvh.antiaim_pitch_dynamic_real_up = true;
  }
  ...
  if(config->hvh.antiaim_pitch){
    if(config->hvh.antiaim_pitch_static){
      ...
      using_fake_pitch = true;
      if(config->hvh.antiaim_pitch_dynamic_real_up)
        global->current_cmd->view_angles.x = swap ? -91.f : 271.f;
      else if(config->hvh.antiaim_pitch_dynamic_real_down)
        global->current_cmd->view_angles.x = swap ? 91.f : -271.f;
    }
  }
    ...
    if(config->hvh.antiaim_pitch_dynamic){
      if(!swap){
        if(global->current_cmd->view_angles.x > 0.f)
          global->current_cmd->view_angles.x = -271.f;
    ...
    if(config->hvh.yaw_method_real_rotate || config->hvh.yaw_method_real_rotate_dynamic){
      ...
      i32 rotation_target = 0;
      {
        if(config->hvh.yaw_rotate_real_target_method_closetargets)
          rotation_target = 1;
        else if(config->hvh.yaw_rotate_real_target_method_snipers)
          rotation_target = 2;
      }
    ...
    if(!config->hvh.antiaim_legit){
      ...
      i32 rotation_target = 0;
      {
        if(config->hvh.yaw_rotate_fake_target_method_closetargets)
          rotation_target = 1;
        else if(config->hvh.yaw_rotate_fake_target_method_snipers)
          rotation_target = 2;
      }
...
void c_antiaim::auto_edge(){
  ...
  i32 rotation_target = 0;
  {
    if(config->hvh.yaw_auto_edge_target_method_closetargets)
      rotation_target = 1;
    else if(config->hvh.yaw_auto_edge_target_method_snipers)
      rotation_target = 2;
  }
...
void c_antiaim::break_animations(bool pre_prediction){
  if(auto_rocket_jumper->is_performing_jump)
    return;
  ...
  {
    s_trace tr = global->trace->player_move(localplayer->origin(), localplayer->origin() + vec3(0.f, 0.f, 24.f), localplayer);
  ...
  if(pre_prediction){
    ...
    global->info_panel->add_entry(INFO_PANEL_ANTIAIM_BREAK_ANIMATIONS, WXOR(L"ACTIVE"), INFO_PANEL_RAGE_CLR);
    set_choke(true);
  }
}
...
void c_antiaim::run_on_fire(){
  ...
  if(weapon->weapon_id() == WPN_FAN || weapon->weapon_id() == WPN_FestiveFAN)
    return;

  global->info_panel->add_entry(INFO_PANEL_ANTIAIM, WXOR(L"FIRING MODE"), INFO_PANEL_WARNING_CLR);
  assert(global->current_cmd != nullptr);
...
void c_antiaim::thirdperson(u32 stage){
  ...
  if(antiaim->is_faking_yaw())
    localplayer->last_viewangle() = vec3(real_pitch_angle, feet_yaw, 0.f);
  else
    localplayer->last_viewangle() = (global->engine->get_latency(true) + global->engine->get_latency(false)) <= math::ticks_to_time(3) ? global->last_sent_angle : localplayer->viewangles();
}
...
bool c_antiaim::should_antiaim(){
  ...
  if(localplayer->is_taunting() || localplayer->has_condition(TF_COND_SHIELD_CHARGE) || localplayer->has_condition(TF_COND_HALLOWEEN_KART))
    return false;
...
bool c_antiaim::get_target_angle(i32 type, vec3* angle, vec3* pos){
  ...
  for(i32 i = 1; i <= globaldata->max_clients; i++){
    ...
    //if(info.fake_player && !utils::is_listen_server())
    //  continue;

    // People who are causing no harm.
    if(player->is_bonked() || player->is_taunting() || player->is_cloaked())
      continue;
    ...
    if(wep->is_harmless_item() || wep->is_melee() || wep->is_wrangler() || wep->is_medigun() || wep->is_flamethrower() || wep->is_projectile_weapon())
      continue;

    if(!localplayer->is_considered_enemy(player))
      continue;

    // We only want snipers with this man..
    if(type == 2){
      if(!player->is_scoped())
        continue;
    }
  ...
  if(best_player != nullptr){
    vec3 predicted_obb = utils::trace_line(best_player->shoot_pos(), best_player->shoot_pos() + (best_player->velocity() * ping), TR_CUSTOM_FILTER_NO_TEAM_BASED_ENTS);
      
    if(pos != nullptr)
      *pos                = best_player->shoot_pos();
    
    if(type != 0)
      *angle              = math::calc_view_angle(localplayer->shoot_pos(), predicted_obb);
    else
...
void c_antiaim::find_headshot_classes(){
  headshot_class = 0;
  if(!config->hvh.antiaim_headshotdanger)
    return;

  float time = math::time();
  headshot_class = global->headshot_threats.get_threats(config->hvh.antiaim_headshotdanger_cheatersonly, config->hvh.antiaim_headshotdanger_vischeck);
  if(headshot_class > 0)
    antiaim_active_headshot_threat = time + 1.f;
}
...
void c_antiaim::process_input(){
  ...
  // Fix config issue.
  if(!config->hvh.antiaim_headshotdanger_any && !config->hvh.antiaim_headshotdanger_cheatersonly)
    config->hvh.antiaim_headshotdanger_any = true;

  find_headshot_classes();
  bool is_active_threat = !config->hvh.antiaim_headshotdanger || config->hvh.antiaim_headshotdanger && headshot_class || config->hvh.antiaim_headshotdanger && antiaim_active_headshot_threat > math::time();
  if(config->hvh.antiaim_usekey){
    ...
    antiaim_enabled = antiaim_toggle && is_active_threat;
    return;
  }

  antiaim_enabled = is_active_threat;
}
...
float c_antiaim::get_best_pitch(c_base_player* localplayer){
  ...
  i32   slot         = weapon->get_slot();
  float speed        = localplayer->velocity().length_2d();
  switch(localplayer->player_class()){
    default: break;
    case TF_CLASS_HEAVY:
    case TF_CLASS_SOLDIER: return slot == 0 ? -91.f : 91.f;
    case TF_CLASS_SCOUT:
    case TF_CLASS_PYRO:
    case TF_CLASS_DEMOMAN:
    case TF_CLASS_SPY:
    case TF_CLASS_ENGINEER: return 91.f;
    case TF_CLASS_MEDIC:
    {
      if(slot == 0 || slot >= 2)
        return 91.f;
      else // The medigun
        return speed <= 25.f ? -91.f : 91.f;
    }
    case TF_CLASS_SNIPER: return -91.f;
  }
...
bool c_antiaim::fake_duck(bool pre_prediction){
  ...
  global->info_panel->add_entry(INFO_PANEL_ANTIAIM_FAKEDUCK, WXOR(L"ACTIVE"), INFO_PANEL_RAGE_CLR);
  if(double_tap->has_any_charged_ticks()){
    global->info_panel->add_entry(INFO_PANEL_ANTIAIM_FAKEDUCK, WXOR(L"NOT POSSIBLE"), INFO_PANEL_WARNING_CLR);
    return false;
  }
...
bool c_antiaim::get_auto_yaw_angle(c_base_player* player, float current_yaw, vec3 target_pos, float radius, std::vector<float> list, float& angle){
  ...
  if(list.size() <= 1){
    for(float z = -math::deg_2_rad(180.f); z <= math::deg_2_rad(180.f); z += math::deg_2_rad(90.f)){
      ...
      s_trace tr1 = global->trace->ray(rotated_pos, target_pos, mask_bullet, TRACE_EVERYTHING, nullptr, TR_CUSTOM_FILTER_NO_TEAM_BASED_ENTS);
      if(tr1.hit_entity == nullptr){
        hit_player = true;
        continue;
      }
  
      s_trace tr2 = global->trace->ray(target_pos, rotated_pos, mask_bullet, TRACE_EVERYTHING, nullptr, TR_CUSTOM_FILTER_NO_TEAM_BASED_ENTS);

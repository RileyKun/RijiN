...
#define RESOLVER_MAX_PITCH_CYCLE 4
#define RESOLVER_MAX_YAW_CYCLE 5
...
public:
  ...
  void float_decode(c_base_entity* entity, float val, bool yaw);
  ...
  void check_movement(c_base_player* player, c_player_data* data);

  void send_resolver_data(c_party_chat_packet_resolver packet);
  void read_resolver_data(c_party_chat_packet_resolver* resolver_data);

  bool is_playerlist_correcting(u8 angle_type, c_base_player* player);
  bool is_good_record(c_base_player* player, s_lc_record* record);
};

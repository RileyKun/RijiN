...
public:
  ...
  virtual c_aimbot_filter_base(){
    ...
    if(localplayer != nullptr){
      ...
      if(localplayer_wep != nullptr){
        ctf_weapon_info* wep_info = localplayer_wep->get_weapon_info();

        if(wep_info != nullptr){
          max_range = wep_info->range;
        }
      }
    }
  }
  ...
  virtual bool handle_priority(s_lc_record* record, c_base_entity* entity){
    ...
    if(!entity->is_player() || !target_ent->is_player())
      return true;
    ...
    c_player_data* target_data  = target_player->data();
    c_player_data* current_data = current_player->data();
...
class c_aimbot_filter_projectile_fov : public c_aimbot_filter_base{
public:
  float closest_fov = 0.f;
...
class c_aimbot_filter_projectile_distance : public c_aimbot_filter_base{
public:
  float closest_distance = 0.f;
  float current_distance = 0.f;
...
class c_aimbot_filter_medigun : public c_aimbot_filter_base{
  public:
  i32 most_lost_hp       = 0;
  i32 best_priority      = 0;
  ...
  bool handle( s_lc_record* record, c_base_player* p ) override;
  bool store( s_lc_record* record, c_base_player* p ) override;
};

class c_aimbot_filter_autorepair : public c_aimbot_filter_base{
public:
  i32   most_lost_hp = 0;

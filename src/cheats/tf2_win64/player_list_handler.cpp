...
void c_player_list_handler::process_cfg_download(){
  ...
  // Avoid rate-limiting, spammage, or blockage from server.
  if(global->is_talking_to_tfcl)
    return;
  ...
  if(fetch_player_data()){
    ...
    notify->create(NOTIFY_TYPE_SUCCESS, WXOR(L"Successfully loaded playerlist data!"));
    return;
  }
  ...
  if(tries >= 3){
    ...
    if(!once){
      notify->create(NOTIFY_TYPE_INFO, WXOR(L"No cloud playerlist data present."));
      once = true;
    }
...
bool c_player_list_handler::fetch_player_data(){
  ...
  static bool once = false;
  if(!once){
    notify->create(NOTIFY_TYPE_INFO, WXOR(L"Fetching playerlist data."));
    once = true;
  }

  return false;
  return cloud::download(PLAYER_LIST_CONFIG_ID, temp_download_buffer, TEMP_DOWNLOAD_BUFFER_SIZE, PLAYER_LIST_CONFIG_VERSION, true, &download_buffer_len) == CLOUD_STATUS_OK;
}
...
bool c_player_list_handler::save_player_data(){
  ...
  // Don't try and save when talking to these.
  if(global->is_talking_to_tfcl)
    return false;
  ...
  utils::create_worker([](void* p){
    ...
    i32 result = 0;;//cloud::upload(PLAYER_LIST_CONFIG_ID, temp_upload_buffer, player_list_handler->raw_cfg_data_size + (sizeof(u32) + sizeof(u32)), true);
    if(result == CLOUD_STATUS_OK){
      DBG("[+] Successfully uploaded playerlist data\n");
    }
    else{
      DBG("[-] Failed to upload playerlist data: %ls\n", cloud::read_response_type(result).c_str());
    }

...
public:
  ...
  u32 get_object_index(c_base_entity* entity){
    ...
    else if(entity->is_whiskey_bottle())
      return OBJECT_TYPE_BOOZE;
    else if(entity->is_weapon_box())
      return OBJECT_TYPE_WEAPON_BOX;
    else if(entity->is_horse())
      return OBJECT_TYPE_HORSE;
  ...
  std::wstring get_object_name(c_base_entity* entity, u32 type){
    ...
    switch(type){
      ...
      case OBJECT_TYPE_BOOZE: return L"Whiskey";
      case OBJECT_TYPE_WEAPON_BOX: return L"Weapon Crate";
      case OBJECT_TYPE_HORSE: return L"Horse";
    }
  ...
  bool should_draw(c_base_player* localplayer, c_base_entity* entity){
    ...
    if(entity->is_whiskey_bottle() || entity->is_weapon_box() || entity->is_horse())
      return true;
    ...
    switch(colour_type){
      case WORLD_ESP_RENDER_TYPE_TEXT:
      {
        ...
        if(config->visual.world_esp_use_team_colours){
          if(entity->team == 2)
            draw_col = flt_array2clr(config->visual.world_esp_vigil_team_colour);
          else if(entity->team == 3)
            draw_col = flt_array2clr(config->visual.world_esp_desper_team_colour);
          else if(entity->team == 4)
            draw_col = flt_array2clr(config->visual.world_esp_bandio_team_colour);
          else if(entity->team == 5)
            draw_col = flt_array2clr(config->visual.world_esp_rangers_team_colour);
          else
            draw_col = flt_array2clr(config->visual.world_esp_unassigned_colour);
        }
        else if(config->visual.world_esp_use_enemyteam_colours){
          if(localplayer->team != entity->team && entity->team >= 2 && entity->team <= 3 || !utils::is_teamplay_enabled())
        ...
        // TYPE LOGIC HERE
        if(type == OBJECT_TYPE_DROPPED_WEAPON || type == OBJECT_TYPE_WEAPON_BOX)
          draw_col = flt_array2clr(config->visual.world_esp_unassigned_colour);
        else if(type == OBJECT_TYPE_BOOZE)
          draw_col = flt_array2clr(config->visual.world_esp_whiskey_colour);
        else if(type == OBJECT_TYPE_GRENADE)
          draw_col = flt_array2clr(config->visual.world_esp_dynamite_colour);
        else if(type == OBJECT_TYPE_HORSE)
          draw_col = flt_array2clr(config->visual.world_esp_horse_colour);
      ...
      case WORLD_ESP_RENDER_TYPE_CHAMS:
      {
        ...
        if(config->visual.world_chams_use_team_colours){
          if(entity->team == 3)
            draw_col = flt_array2clr(config->visual.world_chams_desper_team_colour);
          else if(entity->team == 2)
            draw_col = flt_array2clr(config->visual.world_chams_vigil_team_colour);
          else if(entity->team == 4)
            draw_col = flt_array2clr(config->visual.world_chams_bandio_team_colour);
          else if(entity->team == 5)
            draw_col = flt_array2clr(config->visual.world_chams_rangers_team_colour);
          else
        ...
        if(type == OBJECT_TYPE_DROPPED_WEAPON || type == OBJECT_TYPE_WEAPON_BOX)
          ...
          draw_col = flt_array2clr(config->visual.world_chams_whiskey_colour);
        else if(type == OBJECT_TYPE_GRENADE)
          draw_col = flt_array2clr(config->visual.world_chams_dynamite_colour);
        else if(type == OBJECT_TYPE_HORSE)
          draw_col = flt_array2clr(config->visual.world_chams_horse_colour);
      ...
      case WORLD_ESP_RENDER_TYPE_CHAMS_OVERLAY:
      {
        ...
        if(config->visual.world_chams_use_team_colours){
          if(entity->team == 3)
            draw_col = flt_array2clr(config->visual.world_chams_overlay_desper_team_colour);
          else if(entity->team == 2)
            draw_col = flt_array2clr(config->visual.world_chams_overlay_vigil_team_colour);
          else if(entity->team == 4)
            draw_col = flt_array2clr(config->visual.world_chams_overlay_bandio_team_colour);
          else if(entity->team == 5)
            draw_col = flt_array2clr(config->visual.world_chams_overlay_rangers_team_colour);
          else
        ...
        if(type == OBJECT_TYPE_DROPPED_WEAPON || type == OBJECT_TYPE_WEAPON_BOX)
          ...
          draw_col = flt_array2clr(config->visual.world_chams_overlay_whiskey_colour);
        else if(type == OBJECT_TYPE_GRENADE)
          draw_col = flt_array2clr(config->visual.world_chams_overlay_dynamite_colour);
        else if(type == OBJECT_TYPE_HORSE)
          draw_col = flt_array2clr(config->visual.world_chams_overlay_horse_colour);
      ...
      case WORLD_ESP_RENDER_TYPE_GLOW:
      {
        ...
        if(config->visual.world_glow_use_team_colours){
          if(entity->team == 3)
            draw_col = flt_array2clr(config->visual.world_glow_desper_team_colour);
          else if(entity->team == 2)
            draw_col = flt_array2clr(config->visual.world_glow_vigil_team_colour);
          else if(entity->team == 4)
            draw_col = flt_array2clr(config->visual.world_glow_bandio_team_colour);
          else if(entity->team == 5)
            draw_col = flt_array2clr(config->visual.world_glow_rangers_team_colour);
          else
        ...
        if(type == OBJECT_TYPE_DROPPED_WEAPON || type == OBJECT_TYPE_WEAPON_BOX)
          ...
          draw_col = flt_array2clr(config->visual.world_glow_whiskey_colour);
        else if(type == OBJECT_TYPE_GRENADE)
          draw_col = flt_array2clr(config->visual.world_glow_dynamite_colour);
        else if(type == OBJECT_TYPE_HORSE)
          draw_col = flt_array2clr(config->visual.world_glow_horse_colour);

...
public:
  ...
  ALWAYSINLINE i32 get_priority_hitbox(){
    ...
    if(priority_hitbox == -1)
      config->aimbot.priority_hitbox[0] = true;
  ...
  bool run(){
    ...
    bool attacking = global->current_cmd->buttons & IN_ATTACK || localplayer->has_dual_weapons() && global->current_cmd->buttons & IN_ATTACK2;
    ...
    if(config->aimbot.auto_shoot || !config->aimbot.aim_mode_on_key){
      ...
      if(localplayer->has_dual_weapons())

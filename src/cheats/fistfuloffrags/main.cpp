...
bool setup(void* device){
  ...
  // Setup trampoline addresses
  {
    ...
    global->util_impacttrace_trp                          = gen_internal->get_pkg(HASH("util_impacttrace"));
    ...
    global->fofplayer_firebullets_hook_trp                = gen_internal->get_pkg(HASH("fofplayer_firebullets_hook"));
    global->cmultiplayeranimstate_resetgesture_hook_trp   = gen_internal->get_pkg(HASH("cmultiplayeranimstate_resetgesture_hook"));
  }
  ...
  // Copy signature addresses and pointers
  {
    ...
    global->fire_bullet                  = gen_internal->get_pkg(HASH("fire_bullet"));
    ...
    global->game_rules                   = gen_internal->get_pkg(HASH("gamerules_addr"));
    assert(global->game_rules != nullptr);
  ...
  // Cvar
  {
    ...
    // FOF Commands
    global->cvar->enforce_var(XOR("fof_sv_obj_warmuptime"), XOR("0"), true, 0.f, 0.f);
  }
  ...
  // Create guis
  {
    ...
    if(global->target_window == nullptr)
      global->target_window = FindWindowW(WXOR(L"Valve001"), WXOR(L"Fistful of Frags"));

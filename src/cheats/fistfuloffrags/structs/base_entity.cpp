...
ALWAYSINLINE std::wstring c_base_entity::get_grenade_name(){
  ...
  switch(name_hash){
    default: break;
    case HASH("models/weapons/w_dynamite_black.mdl"):
      return WXOR(L"Black Dynamite");
    case HASH("models/weapons/w_dynamite.mdl"):
      return WXOR(L"Dynamite");
  }
  return L"";
}

ALWAYSINLINE bool c_base_entity::is_whiskey_bottle(){
  s_client_class* cc = get_client_class();
  if(cc == nullptr){
    assert(false);
    return false;
  ...
  return name_hash == HASH("models/items_fof/whiskey_world.mdl");
}
...
bool c_base_entity::is_considered_enemy(c_base_entity* target){
  ...
  // Check teams if teamplay is enabled.
  if(utils::is_teamplay_enabled()){
    // should target team.
    if(config->aimbot.target_friendlies && team == target->team)
      return true;

    // should target enemies
    if(config->aimbot.target_enemies && team != target->team)
      return true;
  }
  else // We're in a free for all now.
    return true;

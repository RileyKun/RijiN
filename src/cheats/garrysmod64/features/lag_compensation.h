...
struct s_lc_record{
  ...
  ALWAYSINLINE bool get_hitbox_pos(c_base_player* player, i32 hitbox, vec3& output_pos){
    return math::get_hitbox_pos(bone_matrix, (c_base_entity*)player, hitbox, output_pos);
  }

  ALWAYSINLINE bool get_hitbox_pos_from_hg(c_base_player* player, i32 hitgroup, vec3& output_pos){
    return math::get_hitbox_pos_from_hg(bone_matrix, (c_base_entity*)player, hitgroup, output_pos);
  }
  ...
  ALWAYSINLINE bool valid(){
    ...
    //float delay = math::time_to_ticks(global->cl_interp->flt());
    {
      // TODO: HANDLE ME!!!
      //if(index == 0)
        //return true;
...
public:
    ...
    for(i32 i = 1; i <= globaldata->max_clients; i++){
      ...
      s_lc_record new_record;
      {
        // bones related
        {
          ...
          if(!result){
            DBG("[-] Failed to setup bones for %s\n", player->info().name);
            continue;
          }
  ...
  bool base_animating_setupbones_run(c_base_player* player, matrix3x4* matrix, i32 max_bones, i32 bone_mask, float current_time){
    ...
    for (u32 i = 0; i < max_bones; ++i){
      float* dest_ptr = (float*)&matrix[i];
      float* src_ptr = (const float*)&record->bone_matrix[i];

      for (u32 j = 0; j < 12; j += 4) {
          // Load 4 floats from the source matrix
          __m128 data = _mm_loadu_ps(&src_ptr[j]);
  
          // Store 4 floats into the destination matrix
          _mm_storeu_ps(&dest_ptr[j], data);
      }
    } 

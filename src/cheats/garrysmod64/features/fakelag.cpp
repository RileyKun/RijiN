...
void c_fakelag::run(){
  ...
  global->info_panel->add_entry(INFO_PANEL_FAKELAG, WXOR(L"FAKELAG"), WXOR(L"INACTIVE"));

 //if(double_tap->has_any_charged_ticks()){
 //  global->info_panel->add_entry(INFO_PANEL_FAKELAG, WXOR(L"FAKELAG"), WXOR(L"NOT POSSIBLE"), INFO_PANEL_WARNING_CLR);
 //  return;
 //}
  ...
  if(ticks_to_break_lc != 0){
    wchar_t buf[256];
    wsprintfW(buf, WXOR(L"CHOKING %i %s"), ticks_to_break_lc, ticks_to_break_lc > 1 ? WXOR(L"TICKS") : WXOR(L"TICK"));
    global->info_panel->add_entry(INFO_PANEL_FAKELAG, WXOR(L"FAKELAG"), buf, INFO_PANEL_RAGE_CLR);
  }
...
void c_fakelag::anti_backtrack(){
  ...
  global->info_panel->add_entry(INFO_PANEL_ANTIBACKTRACK_FAKELAG, WXOR(L"ANTI-BACKTRACK"), WXOR(L"INACTIVE"), INFO_PANEL_WARNING_CLR);
  ...
  if(!global->anti_backtrack_choke){
    for(u32 i = 1; i <= globaldata->max_clients; i++){
      ...
      if(wep->is_melee()){ // Handle logic against players holding melee weapons.
        ...
        s_trace tr1 = global->trace->ray(vec3(furthest_origin.x, furthest_origin.y, furthest_origin.z + 45.f), player->shoot_pos(), mask_shot, TRACE_EVERYTHING, nullptr, TR_CUSTOM_FILTER_NO_TEAM_BASED_ENTS);
        s_trace tr2 = global->trace->ray(shoot_pos, player->shoot_pos(), mask_shot, TRACE_EVERYTHING, nullptr, TR_CUSTOM_FILTER_NO_TEAM_BASED_ENTS);
    ...
    global->info_panel->add_entry(INFO_PANEL_ANTIBACKTRACK_FAKELAG, WXOR(L"ANTI-BACKTRACK"), WXOR(L"ACTIVE"), INFO_PANEL_RAGE_CLR);
...
void c_fakelag::peek_fakelag(){
 ...
 // if(double_tap->is_shift_tick() || double_tap->has_any_charged_ticks())
  ...
  if(global->peek_fakelag_choke){
    global->info_panel->add_entry(INFO_PANEL_PEEK_FAKELAG, WXOR(L"PEEK FAKELAG"), WXOR(L"ACTIVE"), INFO_PANEL_RAGE_CLR);
    ...
    for(i32 index = 1; index <= globaldata->max_clients; index++){
      ...
      s_trace tr = global->trace->ray(localplayer->shoot_pos(), player->shoot_pos(), mask_shot, TRACE_EVERYTHING, nullptr, TR_CUSTOM_FILTER_NO_TEAM_BASED_ENTS);
      if(!tr.vis(player))
        continue;
    ...
    for(i32 index = 1; index <= globaldata->max_clients; index++){
      ...
      s_trace tr = global->trace->ray(shoot_pos, player->shoot_pos(), mask_shot, TRACE_EVERYTHING, nullptr, TR_CUSTOM_FILTER_NO_TEAM_BASED_ENTS);
      if(!tr.vis(player))
        continue;

      s_trace tr2 = global->trace->ray(localplayer->shoot_pos(), player->shoot_pos(), mask_shot, TRACE_EVERYTHING, nullptr, TR_CUSTOM_FILTER_NO_TEAM_BASED_ENTS);
      if(tr2.vis(player))
        continue;
...
bool c_fakelag::active(){
  return true;
  //return /*!double_tap->is_shifting && double_tap->shift_end + math::time_to_ticks(0.5f) < global->original_cmd.command_number;*/
}

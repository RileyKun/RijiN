...
public:
  ...
  ALWAYSINLINE bool should_target_entity(c_base_entity* entity){
    ...
    if(entity->is_player()){
      ...
      if(entity->origin() == vec3(0.f, 0.f, 0.f))
        return false;
  ...
  ALWAYSINLINE i32 get_priority_hitbox(){
    ...
    if(priority_hitbox == -1)
      config->aimbot.priority_hitbox[1] = true;
  ...
  bool get_best_hitbox_point(i32 hitbox, vec3 shoot_pos, c_base_entity* target, s_lc_record* lc_record, vec3& aim_point){
    ...
    c_base_entity* vehicle = nullptr;
    bool is_player = target->is_player();
    if(is_player){

      // If they're in a vehicle aim at their stomach.
      vehicle = target->get_vehicle();
      if(vehicle != nullptr)
        hitbox = 3;

      aim_point = target->obb_center();
      if(lc_record != nullptr)
        lc_record->get_hitbox_pos_from_hg((c_base_player*)target, hitbox, aim_point);
    }
    else{
      c_entity_data* entity_data = target->data();
      if(entity_data == nullptr)
        return false;

      aim_point = target->obb_center();
      if(entity_data->has_bones && entity_data->has_bone_matrix){
        // Prefer priority hitbox(uses hitgroup positions), If fail -> Tries the head, then the stomach, then hitbox 0, and all else fails the obb position.
        if(!math::get_hitbox_pos_from_hg(entity_data->bone_matrix, target, get_priority_hitbox(), aim_point)){
          if(!math::get_hitbox_pos_from_hg(entity_data->bone_matrix, target, 1, aim_point))
            if(!math::get_hitbox_pos_from_hg(entity_data->bone_matrix, target, 3, aim_point))
              if(!math::get_hitbox_pos(entity_data->bone_matrix, target, 0, aim_point))
                aim_point = target->obb_center();
        }
      }
    }
    ...
    // Apply vehicle to ignore filter so we can aimbot the player.
    c_base_entity* ignore_filter = nullptr;
    if(is_player && config->aimbot.trace_filter_skip_vehicle)
      ignore_filter = vehicle;


    s_trace tr = global->trace->ray(shoot_pos, aim_point, mask_shot, TRACE_EVERYTHING, ignore_filter);
    if(tr.vis(target))
      return true;
    ...
    if(is_first_tick){
      if(config->aimbot.autowall_enabled){
        float dmg_out = 0.f;
        if(autowall->m9k_simulate_penetration(target, shoot_pos, aim_point, &dmg_out) && dmg_out > 0.f){
          float hp_f = ((i32)target->health) * ((float)config->aimbot.autowall_min_dmg / 100.f);
          return dmg_out >= hp_f;
        }
      }
    }
  ...
  bool should_run_aimbot(c_base_player* localplayer){
    if(localplayer == nullptr)
      return false;
    ...
    if(!config->aimbot.silent_aim_none && !(localplayer->can_fire() || wep->is_rpg_guiding_missile())  && acp->is_blocked(FUNC_SILENT_AIM))
      return false;
  ...
  bool run(){
    ...
    if(!config->visual.aimbot_target_visual_pred && !should_run_aimbot(localplayer))
      return false;
    ...
    if(config->visual.aimbot_target_visual_pred && !should_run_aimbot(localplayer)){
      global->aimbot_target_reset_time = math::time() + math::ticks_to_time(2);
      aim_lock_target = nullptr;
      return false;
    }

    if(utils::wait_lag_fix(target)){
      global->info_panel->add_entry(INFO_PANEL_AIMBOT, WXOR(L"AIMBOT"), WXOR(L"ACCURATE UNLAG ACTIVE"), INFO_PANEL_WARNING_CLR);
      return false;
    }
    ...
    // set our viewangle
    {
      ...
      if(localplayer->will_fire_this_tick() || acp->is_blocked(FUNC_SILENT_AIM) || weapon->is_rpg_guiding_missile()){
        global->current_cmd->view_angles = aim_angle;

        if(config->aimbot.silent_aim_serverside && localplayer->is_can_fire_supported())
      ...
      if(config->aimbot.silent_aim_none || acp->is_blocked(FUNC_SILENT_AIM))

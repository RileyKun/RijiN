...
//    utils::virtual_call<27, void, float*>(this, c);
//  }
//
...
*/
  ...
  void create_all_required_materials(){
    ...
    // glow related
    {
      ...
      if(material_glow_colour == nullptr){
        ...
        kv->set_string(XOR("$basetexture"), XOR("white"));
      ...
      if(texture_glow_buffer1 == nullptr)
        texture_glow_buffer1 = texture_render_target_frame;
      
      if(texture_glow_buffer2 == nullptr)
        texture_glow_buffer2 = global->material_system->find_texture(XOR("_rt_FullScreen"), XOR("RenderTargets"));;
      

      if(material_glow_blur_x == nullptr){
        c_keyvalues* kv = create_shader_kv(XOR("BlurFilterX"));

        kv->set_string(XOR("$basetexture"), XOR("_rt_FullFrameFB")); // TGB1
      ...
      if(material_glow_blur_y == nullptr){
        ...
        kv->set_string(XOR("$basetexture"), XOR("_rt_FullScreen")); // TGB2
        kv->set_int(XOR("$bloomamount"), 2);
        kv->set_bool(XOR("$ignorez"), true);
  ...
  ALWAYSINLINE void set_rendered_this_frame(c_base_entity* entity, bool rendered, bool set_for_children = false){
    if(entity == nullptr)
      return;
  ...
  c_material* get_material_for_entity(s_chams_context* ctx, bool overlay){
    ...
    */
    ...
    if(ctx->type & CHAMS_TYPE_PLAYER){
      ...
      if(!overlay){
        if(config->world_chams.base_material_none)
          return nullptr;

        if(config->world_chams.base_material_shaded)
          return material_base_default;

        if(config->world_chams.base_material_flat)
          return material_base_flat;
      }
      else{
        if(config->world_chams.overlay_material_none)
          return nullptr;

        if(config->world_chams.overlay_material_flat)
          return material_base_flat;

        if(config->world_chams.overlay_material_fresnel)
          return material_fresnel_overlay;
      }
    }
  ...
  bool get_ignore_z_for_entity(s_chams_context* ctx, bool overlay){
    ...
    if(ctx->type & CHAMS_TYPE_PLAYER){
      ...
      return config->world_chams.ignore_z;
    }
  ...
  bool should_render_team(c_base_entity* entity, bool glow = false){
    ...
    float dist = (float)entity->origin().distance_meter(localplayer->origin());
    config->world_glow.world_glow_render_dist = math::clamp(config->world_glow.world_glow_render_dist, 10.f, 100.f);
    config->world_chams.world_chams_render_dist = math::clamp(config->world_chams.world_chams_render_dist, 10.f, 100.f);

    if(!glow){
      if(entity->is_npc_or_human() || entity->is_infected()){
        ...
        if(!config->world_chams.enabled)
          return false;

        if(dist >= config->world_chams.world_chams_render_dist)
          return false;

        if(entity->is_dropped_weapon() || entity->is_objective())
          return true;

        if(config->world_chams.render_friendies && localplayer->team()== entity->team())
          return true;

        if(config->world_chams.render_enemies && localplayer->team()!= entity->team())
          return true;
      ...
      if(entity->is_npc_or_human() || entity->is_infected()){
        c_base_player* player = (c_base_player*)entity;
        if(player == localplayer){
          if(config->visual.player_glow_render_localplayer)
            return utils::is_in_thirdperson();
        ...
        return dist < config->world_glow.world_glow_render_dist;
  ...
  bool should_render_extra_entity(c_base_entity* entity, u32 object_index, bool glow = false){
    if(entity == nullptr)
      return false;
    ...
    if(glow){
      return config->world_glow.object_enabled[object_index];
    }
    else
      return config->world_chams.object_enabled[object_index];
  ...
  chams_type get_chams_type(c_base_entity* entity){
    ...
    if(entity->is_npc_or_human() || entity->is_infected()){
      ...
      if(should_render_extra_entity(entity, object_index, true) && should_render_team(entity, true) && (config->visual.glow_enabled && config->world_glow.enabled))
        flags |= CHAMS_TYPE_GLOW;
...
*/
  ...
  bool handle_base_material(s_chams_context& ctx){
    ...
    if(ctx.entity != nullptr){
      if(ctx.entity->is_npc_or_human() || ctx.entity->is_infected())
  ...
  bool handle_backtrack_chams(c_base_player* entity, void* ecx, void* edx, void* state, s_model_render_data* info){
    /*
    ...
    utils::call_fastcall<void, void*, s_model_render_data*, matrix3x4*>(gen_internal->decrypt_asset(global->draw_model_execute_hook_trp), ecx, edx, state, info, (void*)&found_record->bone_matrix[0]);
  ...
  bool handle_fake_angle_chams(c_base_player* entity, void* ecx, void* edx, void* state, s_model_render_data* info, matrix3x4* matrix_out){
    ...
    utils::call_fastcall<void, void*, s_model_render_data*, matrix3x4*>(gen_internal->decrypt_asset(global->draw_model_execute_hook_trp), ecx, edx, state, info, (void*)&bone_matrix[0]);
    global->render_view->set_colour_modulation(old_colour);
    set_alpha(entity, old_blend, false);
  ...
  void run_glow_start(c_render_context* rc, s_stencil_state& stencil_state){
    ...
    if(texture_glow_buffer1 == nullptr || texture_glow_buffer2 == nullptr)
      return;
  ...
  void force_material_override(c_material* material){

...
namespace math{
  ...
  inline float get_lerp(){
    ...
    float min_ratio = math::biggest(1.f, global->sv_client_min_interp_ratio->value.flt);
    float ratio     = math::clamp(global->cl_interp_ratio->value.flt, min_ratio, global->sv_client_max_interp_ratio->value.flt);
    ...
    float lerp = math::smallest(ratio / global->cl_updaterate->value.flt, 1.0f);
    float interp = math::smallest(global->cl_interp->value.flt, 1.0f);
...
*/
  ...
  inline void set_random_seed(u32 seed){
    ...
    utils::call_cdecl<void, u32>(random_seed_ptr, seed);
  }
  ...
  inline i32 random_int(i32 min, i32 max){
    ...
    return utils::call_cdecl<i32, i32, i32>(random_int_ptr, min, max);
  }

  inline float random_float(float min, float max){
    static void* random_float_ptr = utils::get_proc_address(utils::get_module_handle(HASH("vstdlib.dll")), HASH("RandomFloat"));
    return utils::call_cdecl<float, float, float>(random_float_ptr, min, max);
  }

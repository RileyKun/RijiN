...
public:
  ...
  bool simulate(vec3 start, vec3 end){
    ...
    // OLD COMMENT: TODO look at FX_FireBullets and see how they reduce weapon damage and the range mod for certain fire types.
    memset(this, 0, sizeof(*this));
    is_simulating       = true;
    {
      // Call entity->FireBullet()
      utils::call_fastcall64<void, vec3, vec3&, float, i32, i32, i32, float, c_base_player*, bool, float, float>(global->fire_bullet, localplayer, start, angles, range, penetration, bullet_type, damage, range_modifier, nullptr, true, 0.f, 0.f);
    }
    is_simulating = false;
    ...
    // Its possibly been fucked by an update.
    assert(dump.damage >= 0.00f && dump.damage <= 1000.f);

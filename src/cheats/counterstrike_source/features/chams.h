...
*/
  ...
  bool is_valid_attachment_entity(c_base_entity* entity, c_base_entity* attachment){
    ...
    if(attachment_cc->id == CBreakableProp)
      return true;
  ...
  bool should_render_team(c_base_entity* entity, bool glow = false){
    ...
    if(!glow){
      ...
      if(entity->is_player()){
        c_base_player* player = (c_base_player*)entity;
        if(config->visual.player_glow_render_steamfriends && player->is_steam_friend())
          return true;
        ...
        if(config->world_glow.render_enemies && entity->team() != localplayer->team())
          return true;
  ...
  NEVERINLINE CFLAG_O0 chams_type get_chams_type(c_base_entity* entity){
    if(entity == nullptr)
...
*/
  ...
  bool handle_backtrack_chams(c_base_player* entity, void* rcx, void* state, s_model_render_data* info){
    ...
    if(weapon->is_grenade() || weapon->is_holding_bomb())
      return false;
    ...
    global->render_view->set_colour_modulation(flt_array2clr(config->visual.backtrack_chams_colour));

...
attrib_managers_sigs = ["55 8B EC 83 EC ? 8B 0D ? ? ? ? 53 56 57 33 FF 33 DB 89 7D ? 89 5D ? 8B 41 ? 85 C0 74 ? 68 ? ? ? ? 68 ? ? ? ? 68 ? ? ? ? 68 ? ? ? ? 6A", "55 8B EC 83 EC ? 8B 0D ? ? ? ? 53 56 57 33 F6 33 FF 89 75 ? 89 7D ? 8B 41 ? 85 C0 74 ? 68 ? ? ? ? 68 ? ? ? ? 68 ? ? ? ? 68 ? ? ? ? 6A"]
#0x1E
...
for sig in attrib_managers_sigs:
  ...
  if cur != idc.BADADDR and cur != 0xffffffff:
    ...
    for x in CodeRefsTo(cur, False):
      ...
      attrib_name = str(ida_bytes.get_strlit_contents(val, -1, ida_nalt.STRTYPE_C))
      if attrib_name == "None" or attrib_name.find("_") == -1:
      ...
      attrib_name = attrib_name.replace("b'","").replace('"','').replace("'","").replace(",","")
      attrib_name = attrib_name.strip()

      #print(attrib_name + ": " + hex(str_offset))
      count = count + 1

      function_name = "None"
      if attrib_name == "back_headshot":
        function_name = "CTFPistol_ScoutSecondary::GetDamageType"
      elif attrib_name == "melee_range_multiplier":
        function_name = "CTFWeaponBaseMelee::DoSwingTraceInternal"
      elif attrib_name == "hwn_mult_postfiredelay":
        function_name = "CTFWeaponBaseGun::PrimaryAttack"
      elif attrib_name == "mult_spread_scale":
        function_name = "CTFWeaponBaseGun::GetWeaponSpread"
      elif attrib_name == "rage_damage":
        function_name = "CTFWeaponBaseGun::GetProjectileDamage"
      elif attrib_name == "building_cost_reduction":
        function_name = "CTFPlayerShared::CalculateObjectCost"
      elif attrib_name == "sniper_fires_tracer":
        function_name = "CTFPlayer::MaybeDrawRailgunBeam"
      elif attrib_name == "set_quiet_unstealth":
        function_name = "CTFPlayerShared::OnRemoveStealthed"
      elif attrib_name == "enables_aoe_heal":
        function_name = "CTFPlayerShared::OnAddTaunting"
      elif attrib_name == "cannot_pick_up_intelligence":
        function_name = "CTFPlayer::IsAllowedToPickUpFlag"
      elif attrib_name == "mult_player_movespeed_resource_level":
        function_name = "CTFPlayer::TeamFortress_CalculateMaxSpeed"
      elif attrib_name == "mult_crit_when_health_is_below_percent":
        function_name = "CTFPlayerShared::IsCritBoosted"
      elif attrib_name == "charge_turn_control":
        function_name = "CTFPlayerShared::CalculateChargeCap"
      elif attrib_name == "hide_enemy_health":
        function_name = "CTargetID::IsValidIDTarget"
      elif attrib_name == "mod_air_control":
        function_name = "CTFGameMovement::GetAirSpeedCap"
      elif attrib_name == "hype_resets_on_jump":
        function_name = "CTFGameMovement::AirDash"
      elif attrib_name == "add_health_regen":
        function_name = "C_TFPlayer::IsHealthBarVisible"
      elif attrib_name == "armor_piercing":
        function_name = "CTFKnife::GetMeleeDamage"
      elif attrib_name == "cannot_be_backstabbed":
        function_name = "CTFKnife::CanPerformBackstabAgainstTarget"
      elif attrib_name == "parachute_attribute":
        function_name = "CTFGameMovement::ToggleParachute"
...
#
## Offset to get the passed string arg
#conmsg_str_offset = [0, 0, 2, 0, 2, 0, 0, 0, 1, 0]
#
#print("Scanning all source engine DBG messages")
...
#
#      # Get the debug text
#      x = x - conmsg_str_offset[pos]
#      warning_txt = str(ida_bytes.get_strlit_contents(idc.get_operand_value(x - 5, 0), -1, ida_nalt.STRTYPE_C))
#      if warning_txt == "None":
#        continue
#

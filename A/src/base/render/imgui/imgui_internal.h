...
#define ImFmod(X, Y)        math::fmod((X), (Y))
#define ImCos(X)            math::cos(X)
#define ImSin(X)            math::sin(X)
#define ImAcos(X)           math::acos(X)
#define ImAtan2(Y, X)       math::atan2((Y), (X))
#define ImAtof(STR)         0.f
//#define ImFloorStd(X)     floorf(X)           // We use our own, see ImFloor() and ImFloorSigned()
#define ImCeil(X)           math::ceil(X)
static inline float  ImPow(float x, float y)    { return 0.f; }          // DragBehaviorT/SliderBehaviorT uses ImPow with either float/double and need the precision
static inline double ImPow(double x, double y)  { return 0.f; }
static inline float  ImLog(float x)             { return 0.f; }             // DragBehaviorT/SliderBehaviorT uses ImLog with either float/double and need the precision
static inline double ImLog(double x)            { return 0.f; }
static inline int    ImAbs(int x)               { return x < 0 ? -x : x; }
static inline float  ImAbs(float x)             { return math::abs(x); }
static inline double ImAbs(double x)            { return math::abs(x); }
static inline float  ImSign(float x)            { return (x < 0.0f) ? -1.0f : (x > 0.0f) ? 1.0f : 0.0f; } // Sign operator - returns -1, 0 or 1 based on sign of argument
static inline double ImSign(double x)           { return (x < 0.0) ? -1.0 : (x > 0.0) ? 1.0 : 0.0; }
#ifdef IMGUI_ENABLE_SSE
static inline float  ImRsqrt(float x)           { return _mm_cvtss_f32(_mm_rsqrt_ss(_mm_set_ss(x))); }
#else
static inline float  ImRsqrt(float x)           { return 1.0f / math::sqrt(x); }
#endif
static inline double ImRsqrt(double x)          { return 1.0 / math::sqrt(x); }
#endif
...
struct TEST
{
    ...
    TEST(ImFontAtlas* shared_font_atlas)
    {
      return;
    }
};

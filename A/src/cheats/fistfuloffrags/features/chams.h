...
*/
  ...
  float get_alpha_for_entity(s_chams_context* ctx, bool overlay){
    ...
    if(ctx->type & CHAMS_TYPE_PLAYER){
      ...
      return overlay ? (settings->overlay_alpha > 0.f ? settings->overlay_alpha / 100.f : 0.f) : (settings->base_alpha > 0.f ? settings->base_alpha / 100.f : 0.f);
    }
    else if(ctx->type & CHAMS_TYPE_ENTITY){
      return overlay ? (config->world_chams.overlay_alpha > 0.f ? config->world_chams.overlay_alpha / 100.f : 0.f) : (config->world_chams.base_alpha > 0.f ? config->world_chams.base_alpha / 100.f : 0.f);
    }
  ...
  bool should_render_team(c_base_entity* entity, bool glow = false){
    ...
    if(!glow){
      if(entity->is_player()){
        ...
        if(entity->is_dropped_weapon() || entity->is_objective() || entity->is_whiskey_bottle() || entity->is_weapon_box() || entity->is_horse())
          return true;

        if(config->world_chams.render_friendies && localplayer->team == entity->team)
          return true;

        if(config->world_chams.render_enemies && (localplayer->team != entity->team || !utils::is_teamplay_enabled()))
          return true;
      ...
      if(entity->is_player()){
        c_base_player* player = (c_base_player*)entity;
        if(config->visual.player_glow_render_steamfriends && (localplayer->is_steam_friend(player) || player->get_index() == global->engine->get_local_player()))
          return true;

        if(config->visual.player_glow_render_friendlies && player->team == localplayer->team)
          return true;

        if(config->visual.player_glow_render_enemies && (localplayer->team != entity->team || !utils::is_teamplay_enabled()))
          return true;
        ...
        if(entity->is_dropped_weapon() || entity->is_objective() || entity->is_whiskey_bottle() || entity->is_weapon_box())
          return true;

        if(config->world_glow.render_friendlies && entity->team == localplayer->team)
          return true;

        if(config->world_glow.render_enemies && (localplayer->team != entity->team || !utils::is_teamplay_enabled()))
          return true;

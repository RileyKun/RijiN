...
c_player_esp_settings* utils::get_player_esp_settings(c_base_player* player){
  ...
  if(localplayer->is_steam_friend(player) || player == localplayer)
    ...
    if(!utils::is_teamplay_enabled())
      type = PLAYER_ESP_SETTING_TYPE_ALLIED_TEAM;
  }
...
bool utils::is_teamplay_enabled(){
  assert(global->game_rules != nullptr);
  c_game_rules* game_rules = **(c_game_rules***)global->game_rules;

  if(game_rules == nullptr){
    DBG("[-] game_rules is nullptr.\n");
    return false;
  }

  return game_rules->is_teamplay_enabled;
}

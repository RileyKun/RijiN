...
#define MAX_SETUP_BONES 256
#define MAXPLAYERS 129
...
struct s_client_state{
  union{
    DATA(0x1C0, i32 server_tick);              // m_nServerTick
    DATA(0x1C4, i32 client_tick);              // m_nClientTick
    DATA(0x1C8, i32 delta_tick);               // m_nDeltaTick
    DATA(0x1CC, i32 signon_state);             // m_nSignonState
    DATA(0x20B78, i32 last_command_ack);       // last_command_ack This is never used, so its not updated.
    DATA(0x20B7C, i32 last_outgoing_command);  // lastoutgoingcommand
    DATA(0x20B80, i32 choked_commands);        // chokedcommands
  };
};
...
struct s_studio_hdr{
  ...
  ALWAYSINLINE s_studio_hitbox_set* get_hitbox_set(i32 i){
    if(i < 0 || i >= num_hitbox_sets){
      //DBG("[-] get_hitbox_set invalid value: %i, returning nullptr.\n", i);
      return nullptr;
    }

...
public:
  ...
  void run(){
    ...
    if(acp->is_blocked(FUNC_MOVE_FIX)){
      global->info_panel->add_entry(INFO_PANEL_AUTOSTRAFE, WXOR(L"AUTOSTRAFE"), WXOR(L"DISABLED FROM PROTECTION"), INFO_PANEL_WARNING_CLR);
      return;
    }

    // do prespeed for jump to gain instant velocity
    if(localplayer->entity_flags & FL_ONGROUND){
      if(config->misc.auto_strafe_prespeed){
        ...
        if(jump_ticks > 0 && jump_ticks <= math::time_to_ticks(0.125f)/*5 TICKS on 66*/ && global->original_cmd.move.length_2d() == 0.f){
          global->current_cmd->move.x = 10000.f;
          global->current_cmd->buttons &= ~IN_JUMP;
          global->info_panel->add_entry(INFO_PANEL_AUTOSTRAFE, WXOR(L"AUTOSTRAFE"), WXOR(L"PRESPEED"), INFO_PANEL_WARNING_CLR);
        }
      }
    ...
    global->info_panel->add_entry(INFO_PANEL_AUTOSTRAFE, WXOR(L"AUTOSTRAFE"), WXOR(L"RUNNING"), INFO_PANEL_RAGE_CLR);
    ...
    if(math::abs(velocity_delta) > ideal_turn){
      ...
      global->current_cmd->move.y        = ideal_mod > 0.f ? -10000.f : 10000.f;
    }
    else{
      global->original_cmd.view_angles.y  = wish_view.y + (ideal * mod);
      global->current_cmd->move.y         = 10000.f * mod;
    }

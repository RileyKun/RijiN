...
void c_player_list_handler::verify_selected_player(){
  ...
  if(global->playerlist_current_player_index == global->engine->get_local_player()){
...
// Copies playerlist data from c_entity_data into player list config.
...
bool c_player_list_handler::fetch_player_data(){
  ...
  return false; //cloud::download(PLAYER_LIST_CONFIG_ID, temp_download_buffer, TEMP_DOWNLOAD_BUFFER_SIZE, PLAYER_LIST_CONFIG_VERSION, true, &download_buffer_len);
}
...
void c_player_list_handler::on_entry_transfer(c_base_player* player, c_entity_data* data){

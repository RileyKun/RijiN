...
class c_procedure_fof : public c_procedure_base{
public:
  ...
  bool write_signatures() override{
   ...
   set_module(HASH("client.dll"));
   {
     ...
     // Not required in games with glow.
     ...
     //add_signature(HASH("fire_bullet"), SIG("\x53\x8B\xDC\x83\xEC\x00\x83\xE4\x00\x83\xC4\x00\x55\x8B\x6B\x00\x89\x6C\x24\x00\x8B\xEC\x81\xEC\x00\x00\x00\x00\xF3\x0F\x2A\x43"), false);
     ...
     add_signature(HASH("prediction_random_seed"), SIG("\xA3\x00\x00\x00\x00\x5D\xC3\x55\x8B\xEC\x51"), false, 0x1);

     // Use the convar "CHudFoFSlide" to find it. There is a string above called "vgui/slide_bg_small" if you see that you're in the correct function.
     // Should be an if statement
     // *(gamerules_21D25510 + 32) that's game rules.
     add_signature(HASH("gamerules_addr"), SIG("\x8B\x0D\x00\x00\x00\x00\xEB\x00\x5F\x5E\x5B"), false, 0x2);
  }
  ...
  bool write_hooks() override{
    ...
    set_module(HASH("client.dll"));
    {
      ...
      add_hook(HASH("base_animating_setupbones_hook"), SIG("\x55\x8B\xEC\x81\xEC\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x53\x8B\xD9\x8B\x0D\x00\x00\x00\x00\x56\x8B\x75"), false);
      ...
      ////v24 = sub_101C61D0();
      //// (*(*v24 + 156))(v24, a2); <-----
      add_hook(HASH("do_post_screen_space_effects_hook"), SIG("\x55\x8B\xEC\x6A\x00\xFF\x75\x00\xB9"), false);
      ...
      // sv_showimpacts l0l
      add_hook(HASH("fofplayer_firebullets_hook"), SIG("\x53\x8B\xDC\x83\xEC\x00\x83\xE4\x00\x83\xC4\x00\x55\x8B\x6B\x00\x89\x6C\x24\x00\x8B\xEC\x81\xEC\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x56\x8B\x73\x00\x57\x8B\xF9\x89\xB5\x00\x00\x00\x00\x89\xBD"), false);

      add_hook(HASH("maintain_sequence_transition_hook"), SIG("\x55\x8B\xEC\x83\xEC\x00\x56\x57\x8B\x7D\x00\x8B\xF1\x8B\xCF"), false);

      // Cancer to find this.
      add_hook(HASH("cmultiplayeranimstate_playflinchgesture_hook"), SIG("\x55\x8B\xEC\x56\x8B\xF1\x8B\x46\x00\x80\x78\x00\x00\x75"), false);

      add_hook(HASH("cmultiplayeranimstate_resetgesture_hook"), SIG("\x55\x8B\xEC\x8B\x55\x00\x56\x8B\xF1\xC1\xE2"), false);
    }

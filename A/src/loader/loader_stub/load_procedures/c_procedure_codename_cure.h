...
class c_procedure_codename_cure : public c_procedure_base{
public:
  ...
  bool write_signatures() override{
   ...
   set_module(HASH("client.dll"));
   {
     ...
     add_signature(HASH("set_abs_angles"), SIG("\x55\x8B\xEC\x83\xEC\x00\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x56\x57\x8B\x7D\x00\x8B\xF1"), false);
   ...
   set_module(HASH("engine.dll"));
   {
     ...
     add_signature(HASH("steam3client_addr"), SIG("\xE8\x00\x00\x00\x00\x8D\x55\x00\x52\x8B\x08\x8B\x01\xFF\x50\x00\x8D\x45\x00\x50\x56\xE8\x00\x00\x00\x00\x83\xC4\x00\x8B\x45"), true);
   }
  ...
  bool write_hooks() override{
    ...
    set_module(HASH("client.dll"));
    {
      ...
      add_hook(HASH("cinput_get_user_cmd_hook"), SIG("\x55\x8B\xEC\x56\x57\x8B\x7D\x00\xB8"), false);
      ...
      // Found using "ERROR: Undefined ammo type!\n"
      add_hook(HASH("cbaseentity_firebullet_hook"), SIG("\x53\x8B\xDC\x83\xEC\x00\x83\xE4\x00\x83\xC4\x00\x55\x8B\x6B\x00\x89\x6C\x24\x00\x8B\xEC\x81\xEC\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x56\x8B\xF1\x57\x8B\x7B\x00\x89\xB5"), false);
    
      // Inside firebullets (required)
      add_hook(HASH("manipulator_applyspread_hook"), SIG("\x55\x8B\xEC\xF3\x0F\x10\x4D\x00\x83\xEC\x00\xF3\x0F\x10\x15"), false);
    }
